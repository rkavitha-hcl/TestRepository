...
absl::Status SetPuntRuleForSut(pdpi::P4RuntimeSession& p4_session,
          ...
          gutil::ParseProtoOrDie<sai::TableEntry>(absl::Substitute(
              R"pb(
                acl_ingress_table_entry {
                  ...
                  action { acl_trap { qos_queue: "0x6" } }  # Action: punt.
                  priority: 1
                }
              )pb"))));
...
TEST_P(SflowTestFixture, CheckIngressSflowSamplesForPuntPackets) {
  // Let SUT punt all packets.
  ASSERT_OK(SetPuntRuleForSut(*sut_p4_session_, GetP4Info(), GetIrP4Info()));
  ...
  LOG(INFO) << "Sample flow count:" << sflow_count;
  const int expected_count =
      (kPacketsNum / kSamplingRateInterval) * kSampleSize / kMaxPacketSize;
