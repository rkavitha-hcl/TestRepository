...
TEST_P(CountersTestFixture, PortCountersTimestamp) {
  ...
  // started.
  ASSERT_OK(gpins::TryUpToNTimes(10, /*delay=*/absl::Seconds(1), [&] {
...
TEST_P(CountersTestFixture, PortQueueCountersTimestamp) {
  ...
  ASSERT_OK_AND_ASSIGN(std::optional<QueueNameByDscp> queue_name_by_ipv4_dscp,
                       GetIpv4DscpToQueueMapping(sut_ingress_port, *gnmi_stub));
  ASSERT_OK_AND_ASSIGN(std::optional<QueueNameByDscp> queue_name_by_ipv6_dscp,
                       GetIpv6DscpToQueueMapping(sut_ingress_port, *gnmi_stub));
