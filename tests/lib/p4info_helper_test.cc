...
TEST(P4InfoHelperTest, TableHasMatchField) {
  ...
  EXPECT_TRUE(TableHasMatchField(ir_p4info, "good_table", "good_match"));
  EXPECT_FALSE(TableHasMatchField(ir_p4info, "bad_table", "good_match"));
  EXPECT_FALSE(TableHasMatchField(ir_p4info, "good_table", "bad_match"));
}
