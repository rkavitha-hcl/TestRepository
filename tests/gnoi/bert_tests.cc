...
TEST_P(BertTest, RunBertOnMaximumAllowedPorts) {
  ...
  ASSERT_NO_FATAL_FAILURE(CheckRunningBertAndForceAdminDown(
      *sut_gnoi_diag_stub, *control_switch_gnoi_diag_stub, *sut_gnmi_stub,
      *control_switch_gnmi_stub, bert_request.bert_operation_id(),
  ...
  ASSERT_NO_FATAL_FAILURE(SetAdminStateOnInterfaceList(
      *sut_gnmi_stub, sut_interfaces_for_admin_down, kEnabledTrue));
  ASSERT_NO_FATAL_FAILURE(SetAdminStateOnInterfaceList(
      *control_switch_gnmi_stub, control_switch_interfaces_for_admin_down,
      kEnabledTrue));
