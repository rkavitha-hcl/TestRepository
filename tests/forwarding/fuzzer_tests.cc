...
bool IsMaskedResource(absl::string_view table_name) {
  ...
  // TODO: unmask when tunnel_table meets resource guarantees.
  return table_name == "acl_egress_table" || table_name == "tunnel_table";
}
...
TEST_P(FuzzerTestFixture, P4rtWriteAndCheckNoInternalErrors) {
  ...
  if (ports.empty()) {
    ...
    ASSERT_OK_AND_ASSIGN(const absl::btree_set<std::string> ports_set,
                         pins_test::GetAllPortIds(GetParam().gnmi_config));
    ports = std::vector<std::string>(ports_set.begin(), ports_set.end());
  }
