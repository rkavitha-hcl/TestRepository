...
TEST_P(L3AdmitTestFixture, L3PacketsAreRoutedOnlyWhenMacAddressIsInMyStation) {
  ...
  ASSERT_OK_AND_ASSIGN(auto sut_ports,
                       GetNUpInterfaceIDs(GetMirrorTestbed().Sut(), 1));
  ASSERT_OK_AND_ASSIGN(
      auto control_ports,
      GetNUpInterfaceIDs(GetMirrorTestbed().ControlSwitch(), 1));
  ...
  ASSERT_OK(
      PuntAllPacketsToController(GetControlP4RuntimeSession(), ir_p4_info()));
  ...
  ASSERT_OK(AddAndSetDefaultVrf(GetSutP4RuntimeSession(), ir_p4_info(),
                                l3_route.vrf_id));
  ASSERT_OK(AddL3Route(GetSutP4RuntimeSession(), ir_p4_info(), l3_route));

  // Admit only 1 MAC address to the forwaring pipeline.
  ASSERT_OK(AdmitL3Route(
      GetSutP4RuntimeSession(), ir_p4_info(),
      L3AdmitOptions{
          .priority = 2070,
  ...
  ASSERT_OK(SendUdpPacket(GetControlP4RuntimeSession(), ir_p4_info(),
...
TEST_P(L3AdmitTestFixture, L3AdmitCanUseInPortToRestrictMacAddresses) {
  ...
  ASSERT_OK_AND_ASSIGN(
      auto control_ports,
      GetNUpInterfaceIDs(GetMirrorTestbed().ControlSwitch(), 2));
...
TEST_P(L3AdmitTestFixture, L3PacketsCanBeClassifiedByDestinationMac) {
  ...
  // DST_MAC.
  if (!TableHasMatchField(ir_p4_info(), "acl_pre_ingress_table", "dst_mac")) {
    GTEST_SKIP()
  ...
  ASSERT_OK(AllowVrfTrafficToDstMac(GetSutP4RuntimeSession(), ir_p4_info(),
                                    good_dst_mac, vrf_id));
  ASSERT_OK(AdmitL3Route(
      GetSutP4RuntimeSession(), ir_p4_info(),
      L3AdmitOptions{
