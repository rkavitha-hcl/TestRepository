...
absl::Status ProgramNextHops(thinkit::TestEnvironment& test_environment,
  ...
  for (const auto& member : members) {
    ...
    ASSIGN_OR_RETURN(p4::v1::TableEntry pi_entry,
                     pdpi::PdTableEntryToPi(ir_p4info, router_interface),
                     _.SetPrepend()
    ...
    auto neighbor_entry =
        gutil::ParseProtoOrDie<sai::TableEntry>(absl::Substitute(
            ...
            "3c:28:6d:34:c0:02"));
    ASSIGN_OR_RETURN(
        pi_entry, pdpi::PdTableEntryToPi(ir_p4info, neighbor_entry),
    ...
    ASSIGN_OR_RETURN(pi_entry, pdpi::PdTableEntryToPi(ir_p4info, nexthop_entry),
                     _.SetPrepend()
                         << "Failed in PD table conversion to PI, entry: "
...
absl::Status ProgramGroupWithMembers(thinkit::TestEnvironment& test_environment,
  ...
  ASSIGN_OR_RETURN(p4::v1::TableEntry pi_entry,
                   pdpi::PdTableEntryToPi(ir_p4info, group_update),
                   _.SetPrepend()
                       << "Failed in PD table conversion to PI, entry: "
...
absl::Status VerifyGroupMembersFromP4Read(
  ...
  ASSIGN_OR_RETURN(auto expected_group_entries,
                   pdpi::PdTableEntryToPi(ir_p4info, pd_group_update),
                   _.SetPrepend()
                       << "Failed in PD table conversion to PI, entry: "
