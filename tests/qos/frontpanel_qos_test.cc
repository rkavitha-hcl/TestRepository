...
absl::Status InstallPdTableEntries(const sai::TableEntries &entries,
  ...
  for (const sai::TableEntry &entry : entries.entries()) {
    ASSIGN_OR_RETURN(pi_entries.emplace_back(),
                     pdpi::PdTableEntryToPi(p4info, entry));
  }
...
TEST_P(FrontpanelQosTest,
  ...
  // Set test case ID.
  testbed->Environment().SetTestCaseID("d748242b-93fe-4cf8-a40e-f5b928909baf");
...
TEST_P(FrontpanelQosTest, WeightedRoundRobinWeightsAreRespected) {
  ...
  // Set test case ID.
  testbed->Environment().SetTestCaseID("948298f5-3ebc-4d92-8446-5ee6e8617502");
    ...
    for (const double kCirsFractionOfEgressLineRate : {0., .40}) {
      ...
      auto delete_traffic_items = absl::Cleanup([&] {
        ...
        for (auto &traffic_item : traffic_items) {
          EXPECT_OK(gpins::TryUpToNTimes(3, absl::Seconds(1), [&] {
...
TEST_P(FrontpanelQosTest, TestWredEcnMarking) {
  ...
  // Set test case ID.
  testbed->Environment().SetTestCaseID("1af5a469-6634-467b-a0fd-19eea425a836");
  ...
  ASSERT_OK_AND_ASSIGN(std::optional<QueueNameByDscp> queue_name_by_ipv4_dscp,
                       GetIpv4DscpToQueueMapping(kSutOutPort, *gnmi_stub));
  ASSERT_OK_AND_ASSIGN(std::optional<QueueNameByDscp> queue_name_by_ipv6_dscp,
                       GetIpv6DscpToQueueMapping(kSutOutPort, *gnmi_stub));
...
TEST_P(FrontpanelBufferTest, BufferCarving) {
  ...
  // Set test case ID.
  if (GetParam().config_to_be_tested == kSharedAlpha) {
    testbed->Environment().SetTestCaseID(
        "408b946e-562c-40fe-9afe-8a7607a3035d");
  }
