  ...
  apply {
    if (local_metadata.apply_tunnel_encap_at_egress) {
      ...
      if (headers.ipv4.isValid()) {
        headers.tunnel_encap_gre.protocol = ETHERTYPE_IPV4;
        ...
        headers.tunnel_encap_ipv6.payload_length =  IPV4_HEADER_BYTES +
                                                    GRE_HEADER_BYTES +
                                      (bit<16>)standard_metadata.packet_length;
      }

      if (headers.ipv6.isValid()) {
        headers.tunnel_encap_gre.protocol = ETHERTYPE_IPV6;
        ...
        headers.tunnel_encap_ipv6.payload_length = IPV6_HEADER_BYTES +
                                                   GRE_HEADER_BYTES +
                                      (bit<16>)standard_metadata.packet_length;
      }
