...
class InstantiationsWithStageTest : public InstantiationTest {};
...
TEST_P(InstantiationsWithStageTest, ProducesPerStageP4Info) {
  EXPECT_THAT(FetchP4Info(GetParam(), ClosStage::kStage2).ByteSizeLong(),
              Gt(0));
  EXPECT_THAT(FetchP4Info(GetParam(), ClosStage::kStage3).ByteSizeLong(),
              Gt(0));
  EXPECT_THAT(FetchP4Info(GetParam(), ClosStage::kStage2),
              Not(EqualsProto(FetchP4Info(GetParam(), ClosStage::kStage3))));
}

INSTANTIATE_TEST_SUITE_P(FetchP4InfoTest, InstantiationsWithStageTest,
                         testing::ValuesIn(InstantiationsWithStage()),
                         InstantiationTestName);
