...
// Given a list of P4Info files, Union them together.
#include <iostream>
#include <string>
...
constexpr char kUsage[] = "--list_of_p4infos=<comma-seperated file list>";
...
int main(int argc, char **argv) {
  ...
  if (absl::GetFlag(FLAGS_list_of_p4infos).empty()) {
    std::cerr
        << "Missing argument: --list_of_p4infos=<comma-seperated file list>"
        << std::endl;
    return 1;
  }
  ...
  for (const std::string &p4info_file : p4info_files) {
    absl::Status status =
        gutil::ReadProtoFromFile(p4info_file, &p4infos.emplace_back());
    if (!status.ok()) {
      std::cerr << "Failed to parse p4info_file: '" << p4info_file << "'."
  ...
  if (!p4info_union.ok()) {
    std::cerr << "Failed to union p4info files:'" << p4info_union.status()
              << "'" << std::endl;
    return 1;
  }
