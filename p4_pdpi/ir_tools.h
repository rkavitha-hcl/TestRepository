...
// in `entry` and apply `transformer` to them.
//
...
// `TransformValuesOfType` uses `info` to find all values of `target_type`
// in `entries` and apply `transformer` to them. See the `TransformValuesOfType`
// which acts on a single entry above for further information.
// If this function returns an error, a prefix of the `entries` may still be
...
// TODO: Add a function for packet in and packet out to callfunction on
// IrPacketMetadata.
