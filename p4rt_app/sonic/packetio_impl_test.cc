...
TEST(PacketIoImplTest, AddAndRemovePacketIoPortWithoutNetdevTranslation) {
  ...
  auto fake_netdev_translator =
      absl::make_unique<FakeIntfTranslator>(/*enabled=*/false);
  PipeFd fd;
  ...
  PacketIoImpl packetio_impl(std::move(mock_call_adapter),
                             std::move(fake_netdev_translator),
                             PacketIoOptions{
...
TEST(PacketIoImplTest, AddAndRemovePacketIoPortWithNetdevTranslation) {
  // SONiC can use / to separate slot/port/channel data, but linux does not
  // support / in interface names. So we will translate netdev names to use _
  // instead.
  const std::string sonic_port_name = "Ethernet1/1";
  const std::string linux_port_name = "Ethernet1_1";

  auto mock_call_adapter = absl::make_unique<sonic::MockSystemCallAdapter>();
  auto fake_netdev_translator =
      absl::make_unique<FakeIntfTranslator>(/*enabled=*/true);
  PipeFd fd;

  // Because we are doing a netdev translation we expect the interface name to
  // convert / to _.
  EXPECT_CALL(*mock_call_adapter, socket).WillOnce(Return(fd.WriteEnd()));
  EXPECT_CALL(*mock_call_adapter, setsockopt).WillOnce(Return(0));
  EXPECT_CALL(*mock_call_adapter, if_nametoindex(StrEq(linux_port_name)))
      .WillOnce(Return(1));
  EXPECT_CALL(*mock_call_adapter, bind).WillOnce(Return(0));
  EXPECT_CALL(*mock_call_adapter, close).WillOnce(Return(0));
  ...
  // After adding check that ports are valid for transmit and receive.
  ASSERT_OK(packetio_impl.AddPacketIoPort(sonic_port_name));
  EXPECT_TRUE(packetio_impl.IsValidPortForTransmit(sonic_port_name));
  EXPECT_TRUE(packetio_impl.IsValidPortForReceive(sonic_port_name));

  // After removing check that ports are not valid for transmit and receive.
  ASSERT_OK(packetio_impl.RemovePacketIoPort(sonic_port_name));
  EXPECT_FALSE(packetio_impl.IsValidPortForTransmit(sonic_port_name));
  EXPECT_FALSE(packetio_impl.IsValidPortForReceive(sonic_port_name));
}
