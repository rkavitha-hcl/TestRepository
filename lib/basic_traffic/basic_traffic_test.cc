...
class FakePacketGenerationFinalizer
 ...
 public:
  FakePacketGenerationFinalizer(thinkit::PacketCallback callback)
      : callback_(std::move(callback)) {}
  void CallCallback(absl::string_view interface, absl::string_view packet) {
    callback_(interface, packet);
  }

 private:
  thinkit::PacketCallback callback_;
};
...
TEST(BasicTraffic, SendTraffic) {
  ...
  EXPECT_CALL(mock_control_device, CollectPackets(_))
      .WillOnce([&fake_finalizer](thinkit::PacketCallback callback)
                    -> absl::StatusOr<
  ...
  EXPECT_CALL(mock_control_device, SendPacket(_, _))
      .WillRepeatedly([&fake_finalizer](absl::string_view interface,
        ...
        // control device.
        fake_finalizer->CallCallback(absl::StrCat("Ethernet1", port_id),
                                     packet);
...
TEST(BasicTraffic, SendTrafficTracksIncorrectlyRoutedPackets) {
  ...
  EXPECT_CALL(mock_control_device, SendPacket(_, _))
      .WillRepeatedly([&fake_finalizer](absl::string_view interface,
        ...
        fake_finalizer->CallCallback("Ethernet10", packet);
        return absl::OkStatus();
      });
